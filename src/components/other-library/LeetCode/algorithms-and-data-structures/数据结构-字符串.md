...
> Create by **jsLe** on **2019-8-22 07:41:30**  
...
> **jsLe** 的联系方式尽在 **jsLe 的文档库**：https://github.com/LiangJunrong/document-library
...
> 注释 1：不强行理解 **顺序存储和链式存储** 的概念是因为 **jsLe** 觉得现在还没有到需要理解的地步。

> 注释 2：如果你纯粹是个知识大师，却敲不了一行代码，那是不行的，所以 **jsLe** 强烈推荐在了解概念的同时多练习练习。
...
- [String - MDN](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/String)
- [JavaScript 中字符串的常见 api 操作（偏 ES5）](https://blog.csdn.net/grapelove01/article/details/80532566)
...
**首先**，由于 **jsLe** 在编写时发现，如果针对某个 API 然后找到对应的某个题给小伙伴尝试，无异于饮鸩止渴，对后期成长是没有帮助的。

**然后**，**jsLe** 尝试攻略的这些题目的覆盖范围可能不是小伙伴想要的（即做了对字符串没法非常深入了解，覆盖面也可能不全），所以小伙伴可以尝试自己选题：

- [LeetCode 字符串专栏](https://leetcode-cn.com/problemset/all/?topicSlugs=string)

**最后**，贴上一些目前 **jsLe** 已经攻略的字符串题目，希望对小伙伴们有所帮助：
